 ...
 * - Fix robusto para recarregar changelog aberto (não depende de texto exato "changelog.txt")
 * - Tratamento de erro em generate/clear changelog (mostra r.error ou fallback)
 * - Higiene de eventos: mantém .off('click.wsp_projects') e usa .off().on() nos seletores críticos
 * - Mantém 100% do código original (sem omissões), apenas corrigindo pontos problemáticos
 */
WSP.projects = {
    /**
    ...
    /**
     * Helper: detecta se o arquivo ativo é o changelog (robusto)
     * - Usa data-path se existir
     * - Remove "* " do nome quando estiver dirty
     */
    _isChangelogOpen: function () {
        ...
        if (p) {
            return (p === 'changelog.txt' || p.endsWith('/changelog.txt'));
        }
    ...
    /**
     * Helper: recarrega o arquivo ativo se for changelog
     */
    ...
    bindEvents: function ($) {
        ...
        $('body').on('click.wsp_projects', '.ctx-add-file', function (e) {
            ...
            WSP.ui.prompt(title, '', function (name) {
                ...
                $.post(window.wspVars.addFileUrl, { project_id: WSP.activeProjectId, name: full }, function (r) {
                    ...
                    else WSP.ui.notify(r.error || WSP.lang('WSP_ERR_INVALID_DATA'), "error");
                }, 'json').fail(function () {
                    WSP.ui.notify(WSP.lang('WSP_ERROR_CRITICAL'), "error");
                });
            });
        ...
        $('body').off('click.wsp_projects', '.generate-project-changelog')
            .on('click.wsp_projects', '.generate-project-changelog', function (e) {
                e.preventDefault();
                if (!WSP.activeProjectId) return;
                ...
                $.post(window.wspVars.changelogUrl, { project_id: WSP.activeProjectId }, function (r) {
                    if (r && r.success) {
                        ...
                        // Recarrega o editor se o changelog estiver aberto (robusto)
        ...
        $('body').off('click.wsp_projects', '.clear-project-changelog')
            .on('click.wsp_projects', '.clear-project-changelog', function (e) {
                e.preventDefault();
                if (!WSP.activeProjectId) return;
