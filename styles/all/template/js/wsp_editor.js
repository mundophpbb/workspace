...
if (window.WSP.projects) {
	// prevents overwriting WSP.projects (duplicate file)
	return;
}
/**
 * Mundo phpBB Workspace - Project Operations
 * Version 4.4 (SSOT + ACL granular): same logic, correct guard per action
 */
WSP.projects = {
	// Internal state for move-file modal actions
	_pendingMove: null,
 ...
 */
	...
	/**
	 * Detects whether the active file is the changelog (robust)
	 */
	...
	/**
	 * Reloads the active file when it is the changelog
	 */
 ...
 */
	_can: function (cap) {
		// Without an active project, no write/actions are allowed (except open-project etc.)
		...
		// Fallback legacy (compat): lock blocks non-admin users
	...
	/**
	 * Notifies permission denied (i18n).
	 */
 ...
 */
	...
	/**
	 * Vincula eventos (delegation e higiene)
	 */
	bindEvents: function ($) {
		var self = this;
		var $body = $('body');
		...
		// Create project (without granular permission in core -> backend validates)
		...
		$body.on('click.wsp_projects', '.rename-active-project', function (e) {
			...
			if (!self._can('manage')) {
				// if management is unavailable, the cause may be a lock or permission
