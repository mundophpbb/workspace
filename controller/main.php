...
class main
{
	...
	/**
	 * Verifica permissão de Fundador (Segurança Máxima)
	 */
	...
	{
		if (!$this->check_founder_permission()) return new JsonResponse(['success' => false, 'error' => $this->user->lang('WSP_ERR_PERMISSION')]);
	...
	public function add_file()
	{
		...
		if (!$project_id || !$filename) return new JsonResponse(['success' => false, 'error' => $this->user->lang('WSP_ERR_INVALID_DATA')]);
	...
	public function add_project()
	{
		...
		if (!$name) return new JsonResponse(['success' => false, 'error' => $this->user->lang('WSP_ERR_INVALID_NAME')]);
	...
	public function search_project()
	{
		...
		if (!$project_id || empty($search_term)) return new JsonResponse(['success' => false, 'error' => $this->user->lang('WSP_ERR_INVALID_DATA')]);
