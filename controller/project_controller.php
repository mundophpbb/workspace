...
class project_controller extends base_controller
{
	...
	public function delete_folder()
	{
		...
		$folder_match_sql = sprintf(
			"(file_name LIKE '%s%%' OR file_name = '%s')",
			$this->db->sql_escape($prefix),
			$this->db->sql_escape($path)
		);
		$files_table = $this->table_prefix . 'workspace_files';

		$this->db->sql_transaction('begin');

		$sql_comments = sprintf(
			'DELETE FROM %sworkspace_comments WHERE project_id = %d AND file_id IN (SELECT file_id FROM %s WHERE project_id = %d AND %s)',
			$this->table_prefix,
			(int) $project_id,
			$files_table,
			(int) $project_id,
		...
		$sql_reviews = sprintf(
			'DELETE FROM %sworkspace_file_reviews WHERE project_id = %d AND file_id IN (SELECT file_id FROM %s WHERE project_id = %d AND %s)',
			$this->table_prefix,
			(int) $project_id,
			$files_table,
			(int) $project_id,
		...
		$sql_versions = sprintf(
			'DELETE FROM %sworkspace_file_versions WHERE project_id = %d AND file_id IN (SELECT file_id FROM %s WHERE project_id = %d AND %s)',
			$this->table_prefix,
			(int) $project_id,
			$files_table,
			(int) $project_id,
		...
		$sql_locks = sprintf(
			'DELETE FROM %sworkspace_file_locks WHERE project_id = %d AND file_id IN (SELECT file_id FROM %s WHERE project_id = %d AND %s)',
			$this->table_prefix,
			(int) $project_id,
			$files_table,
			(int) $project_id,
		...
		$sql_tasks = sprintf(
			'DELETE FROM %sworkspace_tasks WHERE project_id = %d AND file_id IN (SELECT file_id FROM %s WHERE project_id = %d AND %s)',
			$this->table_prefix,
			(int) $project_id,
			$files_table,
			(int) $project_id,
		...
		$sql = sprintf(
			'DELETE FROM %s WHERE project_id = %d AND %s',
			$files_table,
			(int) $project_id,
			$folder_match_sql
		);
	...
	public function rename_folder()
	{
		...
		$files_table = $this->table_prefix . 'workspace_files';
		$new_match_sql = sprintf(
			"(file_name LIKE '%s%%' OR file_name = '%s')",
			$this->db->sql_escape($new_prefix),
			$this->db->sql_escape($new_path)
		);
		$old_match_sql = sprintf(
			"(file_name LIKE '%s%%' OR file_name = '%s')",
			$this->db->sql_escape($old_prefix),
			$this->db->sql_escape($old_path)
		);

		$sql_conflict = sprintf(
			'SELECT file_id FROM %s WHERE project_id = %d AND %s AND NOT %s',
			$files_table,
			(int) $project_id,
			$new_match_sql,
			$old_match_sql
		...
		$sql = sprintf(
			'SELECT file_id, file_name FROM %s WHERE project_id = %d AND %s',
			$files_table,
			(int) $project_id,
			$old_match_sql
		);
	...
	/**
	 * Handles configure access group.
	 */
	...
	/**
	 * Adds user to workspace access group.
	 */
	...
	public function download_project($project_id = 0)
	{
		...
		return $this->create_zip_download_response($temp_file, $download_name);
	}


	/**
	 * Build a phpBB Extension Database submission package.
	 *
	 * The project is stored internally with files relative to the extension root.
	 * Submission packages, however, must contain vendor/extension/... at ZIP root.
	 */
	public function download_submission_package($project_id = 0)
	{
		if ($r = $this->ensure_workspace_access()) { trigger_error($this->user->lang('WSP_ERR_PERMISSION')); }
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$relative = $this->sanitize_rel_path((string) $row['file_name']);
			if ($relative === '' || strtolower(basename($relative)) === '.placeholder')
			{
				continue;
			}
			$rows[] = ['name' => $relative, 'content' => (string) $row['file_content']];
			if ($relative === 'composer.json')
			{
				$composer_raw = (string) $row['file_content'];
			}
		}
		$this->db->sql_freeresult($result);

		$composer = json_decode($composer_raw, true);
		$package_name = is_array($composer) && !empty($composer['name']) ? strtolower(trim((string) $composer['name'])) : '';
		if (!preg_match('#^[a-z][a-z0-9_]*/[a-z][a-z0-9_]*$#', $package_name))
		{
			trigger_error($this->user->lang('WSP_ERR_SUBMISSION_COMPOSER'));
		}

		$version = is_array($composer) && !empty($composer['version']) ? (string) $composer['version'] : 'release';
		$version = preg_replace('/[^a-zA-Z0-9._-]/', '_', $version);
		$base_dir = $package_name . '/';

		$zip = new \ZipArchive();
		$temp_file = tempnam(sys_get_temp_dir(), 'wsp_extdb');
		...
		foreach ($rows as $row)
		{
			...
			if (preg_match('#(?:^|/)(?:\.git|\.github|\.idea|\.vscode)(?:/|$)|(?:^|/)(?:\.DS_Store|Thumbs\.db)$|\.(?:bak|old|orig|rej|log)$#i', $name))
			{
				continue;
			}
			$zip->addFromString($base_dir . $name, (string) $row['content']);
		}
		$zip->close();

		$download_name = str_replace('/', '_', $package_name) . '_' . $version . '.zip';
		return $this->create_zip_download_response($temp_file, $download_name);
	}


	/**
	 * Exports collaborative package.
	 */
	...
	/**
	 * Build a binary ZIP download response and remove the temporary file after sending.
	 */
	private function create_zip_download_response($temp_file, $download_name)
	{
	...
	private function collect_release_validation_rows($project_id)
	{
		...
		foreach ($files as $name => $content)
		{
			...
			if (preg_match('/(?:\/\/+|#+|\/\*+|\*+|<!--)\s*@?(?:TODO|FIXME)\b(?:\s*[:(\[\-]|\s+|$)/', $content))
			{
				$lines = preg_split('/\R/u', $content);
				foreach ($lines as $line_index => $line_text)
				{
					// Only conventional uppercase markers at the start of comment
					// text count as release TODO/FIXME items.
					if (!preg_match('/(?:\/\/+|#+|\/\*+|\*+|<!--)\s*@?(TODO|FIXME)\b(?:\s*[:(\[\-]|\s+|$)/', $line_text, $todo_match))
					{
						continue;
					}
	...
	/**
	 * Handles append changelog entry.
	 */
